...
public class SchedulerService {
    ...
    public static Trigger buildTrigger(final Class jobClass, final SchedulerRest info) {
        ...
        TriggerBuilder<Trigger> cronTriggerTriggerBuilder = TriggerBuilder
                .newTrigger()
                .withIdentity(jobClass.getSimpleName())
                .startAt(new Date(info.getStartTime()));

        switch (info.getScanType()){
            case DAILY -> cronTriggerTriggerBuilder
                    .withSchedule(
                            CronScheduleBuilder
                                    .dailyAtHourAndMinute(
                                            getHoursMinutes(info.getTime())[0],
                                            getHoursMinutes(info.getTime())[1]
                                    )
                    );
            case WEEKLY -> cronTriggerTriggerBuilder
                    .withSchedule(
                            CronScheduleBuilder
            ...
            case MONTHLY -> cronTriggerTriggerBuilder
                    .withSchedule(
                            CronScheduleBuilder
            ...
            case INTERVAL -> cronTriggerTriggerBuilder
                    .withSchedule(
                            SimpleScheduleBuilder
                                    .simpleSchedule()
                                    .repeatForever()
                                    .withIntervalInSeconds(
                                            Math.toIntExact(info.getInterval())
                                    )
                    );
        }
